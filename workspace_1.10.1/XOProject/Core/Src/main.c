...
// Save Data in Here
int MapSoilds[10] = { -1, -1, -1, -1, -1, -1, -1, -1, -1 }; // Max 10 staps
...
int main(void) {
	...
	while (1) {
		...
		/*
		 ...
		 sprintf(Get, "len: %i     ", Length);
		 PrintLCD(60, 24, Get, 0);
		 sprintf(Get, "ms: %i      ", MapSoilds[Length]);
		 PrintLCD(60, 36, Get, 0);
		 sprintf(Get, "lndx: %i    ", IndexSoild);
		 PrintLCD(60, 48, Get, 0);
...
void NextPoint() {
	...
	// Check the Main Arr > Max Len (8) then -> GameOver
	if (GetMaxVal > 9) {
		IndexSoild = 10;
	} else {
		int Max = 1;
		...
		if (Max > 9)
			Max = 1; // If it is greater than the number of cubes, return the old value = zero
		else
		...
		if (IndexSoild != 9) {
			if ((IndexSoild + 1) == 10) {
				...
				Max = IndexSoild + 1; // Note: We do not need this variable anymore (GetMaxVal)
		}
...
void addStapOX() {
	//It is known if the place was used or not
	if (CheckIndexGrid(IndexSoild) == 0) {
		int Length = GetLenInt(MapSoilds);
		if (MapSoilds[Length] == -1) {
			MapSoilds[Length] = IndexSoild;
			struct GameOver GameStatus = Play(IndexSoild, player);

			if (GameStatus.WinPlayer !=0 )
			{
				IndexSoild = 1;
				MapSoilds[0] = -1;
				MapSoilds[1] = -1;
